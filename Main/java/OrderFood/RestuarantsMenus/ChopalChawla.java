...
public class ChopalChawla extends Activity implements AdapterView.OnItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //change_this
        setContentView(R.layout.activity_chopal_chawla);
...
//newly_added
        ...
        list_adapter=new AndroidListAdapter(ChopalChawla.this, R.layout.list_item, DataCollection.data_arr,sharedpreferences,editor);


        ListSeparateAdapter<DataCollection> listsectionAdapter = new ListSeparateAdapter<DataCollection>(ChopalChawla.this,
    ...
    public void onBackPressed()
    {
        finish();
        //change_this
        startActivity(new Intent(getApplicationContext(), ChopalChawlaSubMenu.class));
    }
...
//fetching costs from the server
        ...
        if(message.equals("0")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Restaurants Special", R.drawable.copalchawla_logo, rate[i]));
        }
        if(message.equals("1")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Tandoori Specialities", R.drawable.copalchawla_logo, rate[i]));
        }

        if(message.equals("2")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Chicken Specialities", R.drawable.copalchawla_logo, rate[i]));
        }
        if(message.equals("3")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Mutton Special", R.drawable.copalchawla_logo, rate[i]));
        }

        if(message.equals("4")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Fish Special", R.drawable.copalchawla_logo, rate[i]));
        }

        if(message.equals("5")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Biryani Special", R.drawable.copalchawla_logo, rate[i]));
        }

        if(message.equals("6")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Soup", R.drawable.copalchawla_logo, rate[i]));
        }

        if(message.equals("7")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Bread Special", R.drawable.copalchawla_logo, rate[i]));
        }

        if(message.equals("8")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Salad", R.drawable.copalchawla_logo, rate[i]));
        }
                //"Veg Specialities"
        if(message.equals("9")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Veg Specialities", R.drawable.copalchawla_logo, rate[i]));
        }
