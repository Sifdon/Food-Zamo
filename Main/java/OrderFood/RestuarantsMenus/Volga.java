...
public class Volga extends Activity implements AdapterView.OnItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //change_this
        setContentView(R.layout.activity_volga);
...
//add_data();
        ...
        list_adapter=new AndroidListAdapter(Volga.this, R.layout.list_item, DataCollection.data_arr,sharedpreferences,editor);


        ListSeparateAdapter<DataCollection> listsectionAdapter = new ListSeparateAdapter<DataCollection>(Volga.this,
    ...
    public void onBackPressed()
    {
        finish();
        //change_this
        startActivity(new Intent(getApplicationContext(), VolgaSubMenu.class));
    }
...
//fetching costs from the server
        ...
        if(message.equals("0")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Restaurant Special", R.drawable.volga_cover, rate[i]));
        }

        //"Snacks",
        if(message.equals("1")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Snacks", R.drawable.volga_cover, rate[i]));
        }
                //"Soups",
        if(message.equals("2")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Soups", R.drawable.volga_cover, rate[i]));
        }
                //"Sath-Sath",
        if(message.equals("3")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Sath-Sath", R.drawable.volga_cover, rate[i]));
        }
               // "Vegetarian",
        if(message.equals("4")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Vegetarian", R.drawable.volga_cover, rate[i]));
        }
                //"Chicken",
        if(message.equals("5")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Chicken", R.drawable.volga_cover, rate[i]));
        }
                //"Mutton",
        if(message.equals("6")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Mutton", R.drawable.volga_cover, rate[i]));
        }
                //"Egg",
        if(message.equals("7")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Egg", R.drawable.volga_cover, rate[i]));
        }
                //"Choice of Rice",
        if(message.equals("8")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i],  "Choice of Rice", R.drawable.volga_cover, rate[i]));
        }
                //"Biryani(with Curd)",
        if(message.equals("9")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Biryani(with Curd)", R.drawable.volga_cover, rate[i]));
        }
                //"Roti & Naan",
        if(message.equals("10")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Roti & Naan", R.drawable.volga_cover, rate[i]));
        }
                //"Tandoori Delight",
        if(message.equals("11")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i],  "Tandoori Delight", R.drawable.volga_cover, rate[i]));
        }
                //"Chinese",
        if(message.equals("12")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Chinese", R.drawable.volga_cover, rate[i]));
        }
