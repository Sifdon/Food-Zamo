...
public class Makkhan extends Activity implements AdapterView.OnItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //change_this
        setContentView(R.layout.activity_makkhan);
...
//add_data();
        ...
        list_adapter=new AndroidListAdapter(Makkhan.this, R.layout.list_item, DataCollection.data_arr,sharedpreferences,editor);


        ListSeparateAdapter<DataCollection> listsectionAdapter = new ListSeparateAdapter<DataCollection>(Makkhan.this,
    ...
    public void onBackPressed()
    {
        finish();
        //change_this
        startActivity(new Intent(getApplicationContext(), MakkhanSubMenu.class));
    }
...
//fetching costs from the server
        ...
        if(message.equals("0")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Chinese", R.drawable.south_express_cover, rate[i]));
        }
                //"Dal",
        if(message.equals("1")) {
            for (int i = 0; i < size; i++)
        ...
        if(message.equals("2")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Kofta", R.drawable.south_express_cover, rate[i]));
        }
                //"Mushroom",
        if(message.equals("3")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i],"Mushroom", R.drawable.south_express_cover, rate[i]));
        }
                //"Raita",
        if(message.equals("4")) {
            for (int i = 0; i < size; i++)
        ...
        if(message.equals("7")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i],  "SeasonalVeg", R.drawable.south_express_cover, rate[i]));
        }
                //"SpecialPaneer",
        if(message.equals("8")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "SpecialPaneer", R.drawable.south_express_cover, rate[i]));
        }
                //"Thali"
        if(message.equals("9")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Thali", R.drawable.south_express_cover, rate[i]));
        }
