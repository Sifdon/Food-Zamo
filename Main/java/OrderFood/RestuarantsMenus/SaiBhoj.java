...
public class SaiBhoj extends Activity implements AdapterView.OnItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //change_this
        setContentView(R.layout.activity_sai_bhoj);
...
//add_data();
        ...
        list_adapter=new AndroidListAdapter(SaiBhoj.this, R.layout.list_item, DataCollection.data_arr,sharedpreferences,editor);


        ListSeparateAdapter<DataCollection> listsectionAdapter = new ListSeparateAdapter<DataCollection>(SaiBhoj.this,
    ...
    public void onBackPressed()
    {
        finish();
        //change_this
        startActivity(new Intent(getApplicationContext(), SaiBhojSubMenu.class));
    }
...
//fetching costs from the server
        ...
        if(message.equals("0")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Restaurant Special", R.drawable.saibhoj_logo, rate[i]));
        }
        //"Dal",
        if(message.equals("1")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Dal", R.drawable.saibhoj_logo, rate[i]));
        }
                //"Papad",
        if(message.equals("2")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Papad", R.drawable.saibhoj_logo, rate[i]));
        }
               // "Raita",
        if(message.equals("3")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i],  "Raita", R.drawable.saibhoj_logo, rate[i]));
        }
                //"RicePulao",
        if(message.equals("4")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i],  "RicePulao", R.drawable.saibhoj_logo, rate[i]));
        }
                //"Salads",
        if(message.equals("5")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Salads", R.drawable.saibhoj_logo, rate[i]));
        }
                //"Snacks",
        if(message.equals("6")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Snacks", R.drawable.saibhoj_logo, rate[i]));
        }
                //"Tandoori",
        if(message.equals("7")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Tandoori", R.drawable.saibhoj_logo, rate[i]));
        }
                //"Thali",
        if(message.equals("8")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Thali", R.drawable.saibhoj_logo, rate[i]));
        }
                //"Vegetable"
        if(message.equals("9")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i],"Vegetable", R.drawable.saibhoj_logo, rate[i]));
        }
