...
public class Albaek extends Activity implements AdapterView.OnItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //change_this
        setContentView(R.layout.activity_albaek);
...
//add_data();
        ...
        list_adapter=new AndroidListAdapter(Albaek.this, R.layout.list_item, DataCollection.data_arr,sharedpreferences,editor);


        ListSeparateAdapter<DataCollection> listsectionAdapter = new ListSeparateAdapter<DataCollection>(Albaek.this,
    ...
    public void onBackPressed()
    {
        finish();
        //change_this
        startActivity(new Intent(getApplicationContext(), AlbaekSubMenu.class));
    }
...
//fetching costs from the server
       ...
       if(message.equals("0")) {
           ...
           if(message.equals("1")) {
               for (int i = 0; i < size; i++)
                   DataCollection.data_arr.add(new DataCollection(item[i], "Mutton", R.drawable.albaek, rate[i]));
           }
           if(message.equals("2")) {
               for (int i = 0; i < size; i++)
                   DataCollection.data_arr.add(new DataCollection(item[i], "Chicken", R.drawable.albaek, rate[i]));
           }
           if(message.equals("3")) {
               for (int i = 0; i < size; i++)
                   DataCollection.data_arr.add(new DataCollection(item[i], "Roti/Rolls", R.drawable.albaek, rate[i]));
           }
