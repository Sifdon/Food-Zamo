...
public class Lucky extends Activity implements AdapterView.OnItemClickListener {
...
//add_data();
        ...
        list_adapter=new AndroidListAdapter(Lucky.this, R.layout.list_item, DataCollection.data_arr,sharedpreferences,editor);


        ListSeparateAdapter<DataCollection> listsectionAdapter = new ListSeparateAdapter<DataCollection>(Lucky.this,
    ...
    public void onBackPressed()
    {
        finish();
        //change_this
        startActivity(new Intent(getApplicationContext(), LuckySubMenu.class));
    }
...
//fetching costs from the server
        ...
        if(message.equals("0")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Biryani Special", R.drawable.south_express_cover, rate[i]));
        }
                //"Chicken Special",
        if(message.equals("1")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Chicken Special", R.drawable.south_express_cover, rate[i]));
        }
                //"Egg Curry",
        if(message.equals("2")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Egg Curry", R.drawable.south_express_cover, rate[i]));
        }
                //"Fish Special",
        if(message.equals("3")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Fish Special", R.drawable.south_express_cover, rate[i]));
        }
               // "Mutton Special",
        if(message.equals("4")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i],  "Mutton Special", R.drawable.south_express_cover, rate[i]));
        }
                //"Roties/Naan",
        if(message.equals("5")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Roties/Naan", R.drawable.south_express_cover, rate[i]));
        }
                //"Tandoori Specialities",
        if(message.equals("6")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Tandoori Specialities", R.drawable.south_express_cover, rate[i]));
        }
                //"Veg Curries"
        if(message.equals("7")) {
            for (int i = 0; i < size; i++)
