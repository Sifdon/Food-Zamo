...
public class Thadka extends Activity implements AdapterView.OnItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //change_this
        setContentView(R.layout.activity_thadka);
...
//add_data();
        ...
        list_adapter=new AndroidListAdapter(Thadka.this, R.layout.list_item, DataCollection.data_arr,sharedpreferences,editor);


        ListSeparateAdapter<DataCollection> listsectionAdapter = new ListSeparateAdapter<DataCollection>(Thadka.this,
    ...
    public void onBackPressed()
    {
        finish();
        //change_this
        startActivity(new Intent(getApplicationContext(), ThadkaSubMenu.class));
    }
...
//fetching costs from the server
        ...
        if(message.equals("0")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Restaurant Special", R.drawable.thadka_cover_pic, rate[i]));
        }
        //"Tandoori Specialities",
        if(message.equals("1")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Tandoori Specialities", R.drawable.thadka_cover_pic, rate[i]));
        }
                //"Chicken Specialities",
        if(message.equals("2")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Chicken Specialities", R.drawable.thadka_cover_pic, rate[i]));
        }
                //"Fish Specialities",
        if(message.equals("3")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Fish Specialities", R.drawable.thadka_cover_pic, rate[i]));
        }
                //"Mutton Specialities",
        if(message.equals("4")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Mutton Specialities", R.drawable.thadka_cover_pic, rate[i]));
        }
                //"Biryani Special",
        if(message.equals("5")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Biryani Special", R.drawable.thadka_cover_pic, rate[i]));
        }
                //"Bread Specialities",
        if(message.equals("6")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Bread Specialities", R.drawable.thadka_cover_pic, rate[i]));
        }
                //"Salad Specialities",
        if(message.equals("7")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Salad Specialities", R.drawable.thadka_cover_pic, rate[i]));
        }
                //"Veg Specialities",
        if(message.equals("8")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Veg Specialities", R.drawable.thadka_cover_pic, rate[i]));
        }
                //"Snacks",
        if(message.equals("9")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Snacks", R.drawable.thadka_cover_pic, rate[i]));
        }
