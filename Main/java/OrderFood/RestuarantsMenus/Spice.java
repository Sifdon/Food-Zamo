...
public class Spice extends Activity implements AdapterView.OnItemClickListener {
...
//add_data();
        ...
        list_adapter=new AndroidListAdapter(Spice.this, R.layout.list_item, DataCollection.data_arr,sharedpreferences,editor);


        ListSeparateAdapter<DataCollection> listsectionAdapter = new ListSeparateAdapter<DataCollection>(Spice.this,
    ...
    public void onBackPressed()
    {
        finish();
        //change_this
        startActivity(new Intent(getApplicationContext(), SpiceSubMenu.class));
    }
...
//fetching costs from the server
        ...
        if(message.equals("0")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Restaurant Special", R.drawable.spice_cover_pic, rate[i]));
        }
        //"Pizzas",
        if(message.equals("1")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Pizzas", R.drawable.spice_cover_pic, rate[i]));
        }
                //"Burgers",
        if(message.equals("2")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Burgers", R.drawable.spice_cover_pic, rate[i]));
        }
                //"Halka Fulka",
        if(message.equals("3")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i],  "Halka Fulka", R.drawable.spice_cover_pic, rate[i]));
        }
                //"Garden Green",
        if(message.equals("4")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Garden Green", R.drawable.spice_cover_pic, rate[i]));
        }
                //"China Town",
        if(message.equals("5")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "China Town", R.drawable.spice_cover_pic, rate[i]));
        }
                //"South Indian",
        if(message.equals("6")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "South Indian", R.drawable.spice_cover_pic, rate[i]));
        }
                //"Dahi Ke Rishtedar",
        if(message.equals("7")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i],  "Dahi Ke Rishtedar", R.drawable.spice_cover_pic, rate[i]));
        }
                //"Rice",
        if(message.equals("8")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Rice", R.drawable.spice_cover_pic, rate[i]));
        }
                //"Tandoori Snacks",
        if(message.equals("9")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i],  "Tandoori Snacks", R.drawable.spice_cover_pic, rate[i]));
        }
                //"Non Vegetarian",
        if(message.equals("10")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Non Vegetarian", R.drawable.spice_cover_pic, rate[i]));
        }
                //"Vegetarian",
        if(message.equals("11")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Vegetarian", R.drawable.spice_cover_pic, rate[i]));
        }
                //"Nan & Roti",
        if(message.equals("12")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Nan & Roti", R.drawable.spice_cover_pic, rate[i]));
        }
