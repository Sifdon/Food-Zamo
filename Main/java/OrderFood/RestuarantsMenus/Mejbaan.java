...
public class Mejbaan extends Activity implements AdapterView.OnItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //change_this
        setContentView(R.layout.activity_mejbaan);
...
//add_data();
        ...
        list_adapter=new AndroidListAdapter(Mejbaan.this, R.layout.list_item, DataCollection.data_arr,sharedpreferences,editor);


        ListSeparateAdapter<DataCollection> listsectionAdapter = new ListSeparateAdapter<DataCollection>(Mejbaan.this,
    ...
    public void onBackPressed()
    {
        finish();
        //change_this
        startActivity(new Intent(getApplicationContext(), MejbaanSubMenu.class));
    }
...
//fetching costs from the server
        ...
        if(message.equals("2")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i],  "Rice & Noodles(Veg)", R.drawable.south_express_cover, rate[i]));
        }
               
        if(message.equals("3")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Non-Veg Delights", R.drawable.south_express_cover, rate[i]));
        }
               // "Lamb",
        if(message.equals("4")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Lamb", R.drawable.south_express_cover, rate[i]));
        }
                //"Rice & Noodles(Non-Veg)",
        if(message.equals("5")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Rice & Noodles(Non-Veg)", R.drawable.south_express_cover, rate[i]));
        }
                //"South Indian",
        if(message.equals("6")) {
            for (int i = 0; i < size; i++)
        ...
        if(message.equals("7")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i],  "Sandwiches", R.drawable.south_express_cover, rate[i]));
        }
                //"Snacks",
        if(message.equals("8")) {
            for (int i = 0; i < size; i++)
        ...
        if(message.equals("9")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Vegetarian Soups", R.drawable.south_express_cover, rate[i]));
        }
               // "Non Vegetarian Soups",
        if(message.equals("10")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Restuarant Special", R.drawable.south_express_cover, rate[i]));
        }
                //"Tandoori Khazana",
        if(message.equals("11")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Tandoori Khazana", R.drawable.south_express_cover, rate[i]));
        }
                //"Veg Starters",
        if(message.equals("12")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Veg Starters", R.drawable.south_express_cover, rate[i]));
        }
                //"Non-Veg Starters",
        if(message.equals("13")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Non-Veg Starters", R.drawable.south_express_cover, rate[i]));
        }
                //"Veg Curries",
        if(message.equals("14")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Veg Curries", R.drawable.south_express_cover, rate[i]));
        }
                //"Non-Veg Curries",
        if(message.equals("15")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Non-Veg Curries", R.drawable.south_express_cover, rate[i]));
        }
                //"Dal",
        if(message.equals("16")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Dal", R.drawable.south_express_cover, rate[i]));
        }
               // "Raita",
        if(message.equals("17")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i],  "Raita", R.drawable.south_express_cover, rate[i]));
        }
               // "Rice",
        ...
        if(message.equals("20")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i],  "Ala-Carte Breakfast", R.drawable.south_express_cover, rate[i]));
        }
                //"Continental Flavours"
        if(message.equals("21")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Continental Flavours", R.drawable.south_express_cover, rate[i]));
        }
