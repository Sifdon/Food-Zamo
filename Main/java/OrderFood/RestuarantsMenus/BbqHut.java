...
public class BbqHut extends Activity implements AdapterView.OnItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //change_this
        setContentView(R.layout.activity_bbq_hut);
...
//add_data();
        ...
        list_adapter=new AndroidListAdapter(BbqHut.this, R.layout.list_item, DataCollection.data_arr,sharedpreferences,editor);


        ListSeparateAdapter<DataCollection> listsectionAdapter = new ListSeparateAdapter<DataCollection>(BbqHut.this,
    ...
    public void onBackPressed()
    {
        finish();
        //change_this
        startActivity(new Intent(getApplicationContext(), BbqHutSubMenu.class));
    }
...
//fetching costs from the server
        ...
        if(message.equals("0")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Restaurant Special", R.drawable.bbqhut, rate[i]));
        }
        if(message.equals("1")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Veg.Barbecue", R.drawable.bbqhut, rate[i]));
        }
                //"Non-Vegetarian",
        if(message.equals("2")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Non-Vegetarian", R.drawable.bbqhut, rate[i]));
        }
                //"Biryani/rice",
        if(message.equals("3")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Biryani/rice", R.drawable.bbqhut, rate[i]));
        }
                //"Paranthas",
        if(message.equals("4")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Paranthas", R.drawable.bbqhut, rate[i]));
        }
                //"Veg.Main Course",
        if(message.equals("5")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Veg.Main Course", R.drawable.bbqhut, rate[i]));
        }
                //"Nonveg Main Course",
        if(message.equals("6")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "NonVeg Main Course", R.drawable.bbqhut, rate[i]));
        }
                //"Roti/Breads",
        if(message.equals("7")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Roti/Breads", R.drawable.bbqhut, rate[i]));
        }
                //"Fish",
        if(message.equals("8")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Fish", R.drawable.bbqhut, rate[i]));
        }
                //"Veg Snacks"
        if(message.equals("9")) {
            for (int i = 0; i < size; i++)
                DataCollection.data_arr.add(new DataCollection(item[i], "Veg Snacks", R.drawable.bbqhut, rate[i]));
        }
